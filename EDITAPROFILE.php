...
<section class="padding container" id="section-1">
    ...
    <div class="  vh-sm-200 vh-md-100 row">
        <div class=" padding col-md-6 col-sm-12">
            <h2><?php echo $result['fname']; ?>'s General Information</h2>
            <form method="post">
                <label for="exampleFormControlInput1" class="form-label">Photo</label>
                <input name="nphoto" type="file" class="form-control"  placeholder="Ex:Salah-pic.png" >
                <label for="exampleFormControlInput1" class="form-label">First Name</label>
                <input name="nfname" type="text" class="form-control"  placeholder="Ex:Salah" value="<?php echo $result['fname']; ?>">
                ...
                <button name="updatebtn" type="submit" class="btn btn-primary">Update this user's informations</button>
            </form>
            ...
            if(isset($_POST['updatebtn']))
            {
                ...
                $sql=mysqli_query($con,"UPDATE `users` SET `fname`='$nfname',`lname`='$nlname',`photo`='$nphoto',`username`='$nusername',`major`='$nmajor',`interests`='$ninterests' WHERE sid='$id' ");
                if($sql == true)
                {
                    echo '
                            <script>
                            alert("User\'s informations updated successfully!");
        ...
        <div class=" padding col-md-6 col-sm-12">
            ...
            <form method="post">
                ...
                <button name="pupdatebtn" type="submit" class="btn btn-primary">Update this user's password</button>
            ...
            {
                $id=$_GET['id'];
                $pswd1=$_POST['npassword'];
                $pswd2=$_POST['npassword2'];
                #$npassword=$_POST['npassword'];
                if($pswd1==$pswd2){
                    $psql=mysqli_query($con,"UPDATE `users` SET `password`='$pswd1' WHERE sid='$id' ");
                    #$sql2=mysqli_query($con,"UPDATE `users` SET `password`='$npassword' WHERE sid='id' ");
                    if($psql == true)
                    {
                        echo '
                            <script>
                            alert("User\'s password successfully updated!");
                            window.location="ADMINUSERS.php";
                            </script>
                            ';
                    }
                ...
                else
                {
                    echo '
                            <script>
                            alert("Something is wrong in the update!\nMake sure the entered new passwords are identical.");
                            </script>
                            ';
                }
            }
